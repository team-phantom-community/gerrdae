...
def tensorflow_processing(fft_queue, stop_event):
    ...
    model.load_weights('./savedweight_len200_0315.weights.h5')
    model.summary()
    
    section = ["non-walk", "walk"]
    while not stop_event.is_set():# or not fft_queue.empty():
        if not fft_queue.empty():
            fft_data = fft_queue.get()
            fft_data = np.array([fft_data])
            #正規化
            data_generalized = fft_data / np.amax(fft_data)
...
if __name__ == "__main__":
    ...
    process3 = mp.Process(target=tensorflow_processing, args=(fft_queue, stop_event))
